...
Two facts drive the design, both measured rather than assumed (``scripts/lora_numerics.py`` and
``scripts/lora_layout_probe.py``):
...
The pruned lineage also **cannot accept the adaLN LoRA**: DeepBeepMeep replaced the
``[96768, 2688]`` per-block timestep projection with a ``[96768, 64]`` projection over a sampled
rank-64 curve, so the ``[16, 2688]`` ``lora_A`` has no input to consume. Those modules are reported
as skipped with the reason, never silently dropped. See :func:`plan` and the note in
``notes/TURBO_LORA_PROBE.md`` for what it would take to absorb them.
"""
